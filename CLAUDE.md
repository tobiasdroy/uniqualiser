...
Rendered by `IntroCard()` in `App.tsx` as two sibling `<section>`s (a fragment, not a single card) at the top of `MainLayout`, above `OscillatorControl`.

**Hero band (`.introHero`)** — full-bleed, breaks out of `.main`'s 1200px column via the `left: 50%; margin-left: -50vw; width: 100vw` technique, flush against the sticky header (`margin-top: -24px` cancels `.main`'s top padding). Background is a **fixed-colour** diagonal gradient (`#7f1d1d` → `#b91c1c` → `#9a3412`, deep red into rust) layered with a few off-corner radial highlights and the SVG grain overlay — hardcoded hex/rgba values, not theme tokens, so the hero renders identically in light and dark mode. Heading and lead paragraph are hardcoded `#ffffff` / `rgba(255,255,255,0.92)` for the same reason (these two classes are hero-only, safe to hardcode). Inner content (`.introHeroInner`, grid `1fr 400px`, single column ≤760px) is re-constrained to max-width 1200px so text stays a readable width. Left column: heading + lead paragraph + a **"Get Started" CTA** (`.heroCta`, white pill button) that anchor-links to `#oscillator` (the `id` lives on the `OscillatorControl` wrapper in `MainLayout`); `html { scroll-behavior: smooth }` in `globals.css` handles the scroll, `#oscillator { scroll-margin-top: 96px }` in `App.module.css` keeps the sticky header from covering it, and both respect `prefers-reduced-motion`. Right column: `HeroVisual`, a decorative SVG echoing the real EQ curve (grid, glow, curve path, three **static** handle dots — no longer float-animated). Purely decorative (`aria-hidden`), not the live EQ curve; its stroke/fill colours are also hardcoded white/deep-red to match the fixed hero background.

**"Why personalise" band (`.introWhy`)** — also full-bleed (same breakout technique), but deliberately **not** a card: no border-radius, shadow, or hover lift. Uses `background: var(--card-bg)` (the same surface tone as the tool cards, so it still reads as visually distinct from the plain page background) with a `border-bottom` seam. `.introHero` carries a `margin-bottom: 16px` — matching `.main`'s flex `gap` — so there's an equal page-background gap on both sides of this band (hero → gap → why-band → gap → Oscillator card). Two-column explainer (single column ≤760px) covering why AutoEQ alone is insufficient (HRTF variation) and the recommended workflow (AutoEQ for sub-1 kHz, Uniqualiser for above), with a link to The Headphone Show YouTube video.

`--accent-secondary` (warm amber) is no longer used in the hero — it read as mismatched against the deep red once the background went fully saturated, so the hero gradient was rebuilt from fixed red/rust hex values instead. `--accent-secondary` is still defined as a token and still decorative-only elsewhere, just unused here now.
...
- **Level Match** — toggles `levelMatch` local state. When active (highlighted), a `useEffect` in `EQBandControl` calls `computeAverageGainDb(filterNodes)` from `frequencyMath.ts` and sets `engine.setBypassTrim(10^(avgDb/20))`. This attenuates the flat bypass signal to match the EQ'd level — the EQ path is never modified, so there is no clipping risk. Recomputes automatically on every band change (and on preamp change). `bypassTrimNode` is only in the bypass path, so when EQ is active the trim has no effect. The button label shows a dB readout, e.g. `Level Match (-5.0 dB)`, computed as **`avgDb + preampGain`** and stored in `levelMatchDb` state — deliberately *not* the same number used for `setBypassTrim` (which stays `avgDb` alone). `masterGainNode` sits downstream of both the filter chain and `bypassTrimNode` (same node, same value, either routing — see `rebuildChain()`), so preamp cancels out of the in-app bypass comparison and must not be added there. But it's exactly the extra gain a *flat preset in external EQ software* would need dialled in to match this profile's overall loudness against an unprocessed signal, which is what the on-screen number is for. Because auto-preamp keeps this profile's peak at ≤ 0 dB vs. the source, this readout is normally ≤ 0 dB too.
...
- The full-bleed hero/why-personalise bands (`left: 50%; margin-left: -50vw; width: 100vw`) rely on `overflow-x: hidden` on `html, body` in `globals.css` — without it, scrollbar-width rounding can introduce a few px of horizontal scroll on some platforms.
- `#oscillator` in `App.module.css` is a plain ID selector, not a class — CSS Modules only scopes/hashes class selectors, so it can target the `id="oscillator"` prop set directly on the `OscillatorControl` wrapper in `MainLayout` without needing `:global()`.
- Don't add `avgDb`'s preamp contribution into `engine.setBypassTrim()` — `masterGainNode` already sits downstream of both the EQ and bypass paths (same node either way, see `rebuildChain()`), so preamp cancels out of that in-app comparison automatically. It only belongs in the separately-tracked `levelMatchDb` display value (see Level Match above).
